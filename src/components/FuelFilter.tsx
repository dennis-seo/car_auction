...
import type { AuctionItem, ActiveFilters, FilterAction } from '../types';

/** 필터 모드 타입 */
type FilterMode = 'vehicleType' | 'fuel';
...
// 차량 용도별 아이콘 (오토허브용)
const VEHICLE_TYPE_ICONS: IconMap = {
    '렌터카': '🚗',
    '자가용': '🏠',
    '업무용': '💼',
    '영업용': '🚕',
 ...
 * @returns 필터 설정 객체
 */
const createFilterConfig = (filterMode: FilterMode): FilterConfig => {
...
/**
 * 기타 그룹에 속하는 값들을 찾습니다.
 * @param groupCounts - 그룹별 개수
 * @param allVariants - 정의된 모든 변형 목록
 * @returns 기타 그룹에 속하는 값들
 */
const getOtherGroupValues = (
 ...
 */
const DynamicFilter: React.FC<DynamicFilterProps> = ({ data, activeFilters, onUpdateFilter }) => {
    // AuctionManager를 통해 필터 모드 결정
    const filterMode = useMemo((): FilterMode => {
        // AuctionManager가 초기화된 경우 해당 결과 사용
        if (auctionManager.isReady()) {
            const mode = auctionManager.getFilterMode() as FilterMode;
        ...
        return data && data.some(row => row.auction_name === "오토허브 경매장")
            ? FILTER_TYPES.VEHICLE_TYPE as FilterMode
            : FILTER_TYPES.FUEL as FilterMode;
    }, [data]);
    ...
    const activeValues = useMemo((): string[] =>
        (activeFilters?.[config.filterKey as keyof ActiveFilters] as string[]) || [],
        [activeFilters, config.filterKey]
    );

    // 그룹 활성화 상태 확인
    const isGroupActive = useCallback((groupLabel: string): boolean => {
        if (groupLabel === '전체') {
    ...
    const isGroupFullyActive = useCallback((groupLabel: string): boolean => {
        if (groupLabel === '기타') {
            const otherValues = getOtherGroupValues(groupCounts, config.allVariants);
    ...
    // 그룹 토글 처리
    const handleToggleGroup = useCallback((groupLabel: string): void => {
