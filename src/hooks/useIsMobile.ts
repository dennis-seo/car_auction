 ...
 * - MediaQueryList API 사용으로 resize 이벤트보다 효율적
 * - 디바운스 적용으로 빈번한 리사이즈 시 성능 향상
 *
 * @param breakpoint - 모바일로 간주할 최대 너비 (기본값: 640)
 * @returns 모바일 여부
 */
export const useIsMobile = (breakpoint: number = 640): boolean => {
    ...
    const debouncedSetIsMobile = useCallback((matches: boolean) => {
    ...
    useEffect(() => {
        ...
        const handler = (e: MediaQueryListEvent) => debouncedSetIsMobile(e.matches);
